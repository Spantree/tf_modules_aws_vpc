...
resource "aws_route_table" "public" {
  ...
  tags             = "${merge(var.public_tags, var.tags, map("Name", format("%s-rt-public", var.name)))}"
}
...
resource "aws_subnet" "public" {
  ...
  tags              = "${merge(var.public_tags, var.tags, map("Name", format("%s-subnet-public-%s", var.name, element(var.azs, count.index))))}"
